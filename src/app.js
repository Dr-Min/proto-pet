...
function ownedFurnitureIds() {
  ...
  const stored = Array.isArray(careStats.furnitureStored) ? careStats.furnitureStored : [];
  return careStats.furnitureOwned.filter(id => furnitureItemById(id) && !stored.includes(id));
}
...
const CAPTIONS = {
  wander: ['나 어디 가?', '잠깐만 볼게', '여기 좋아', '발이 먼저 나감', '바닥 순찰 중', '괜히 돌아보는 중'],
  stare: ['뭐 해?', '나 봤어?', '그냥 같이 있자', '멍 (본인생각)', '눈만 켜둠', '아무 생각 없는 척'],
  sniff: ['킁킁', '여기 뭐 있어', '냄새 좋다', '코 바쁨', '바닥 조사 중', '수상한 데 발견'],
  plop: ['나 좀 누울게', '여기 시원해', '움직이기 싫어', '철푸덕', '꼬물꼬물 녹는 중', '바닥이랑 합체 중'],
  zoomies: ['나 빨라!', '봤어?', '우다다다!!', '갑자기 신남', '발 난리 중', '속도 자랑 중'],
  wiggle: ['이거 춤이야', '나 잘하지', '꼬물꼬물', '춤 (본인생각)', '몸이 먼저 신남', '흔들림 연습 중'],
  sleep: ['잘래…', '조금만 잘게', '꿈에서 봐', '자는 척', '눈 꺼지는 중', '꿈나라 입장 중'],
...
const PICKUP_LINES = ['어? 나?', '안아줘?', '잠깐만', '높다', '나 지금 떠?', '들림', '발 사라진 중', '공중 산책 중', '바닥 멀어지는 중', '손에 수거됨'];
const CARRY_LINES = ['어디 가?', '나 여기 있어', '천천히 가', '주인 손이다', '공중 구경 중', '둥둥 상태', '다리 쉬는 중', '바닥 놓친 중', '몸이 길어지는 척', '들린 채 생각 중'];
const DROP_LINES = ['어어 내려간다', '살살 놔줘', '후웅', '착지할게', '바닥 온다', '숨 참는 중', '말랑 준비 중', '다리 찾는 중', '아래로 배송 중', '착지 시뮬레이션 중'];
const LAND_LINES = ['살았다', '콩', '땅 좋아', '나 괜찮아', '다시 해도 돼', '착지 성공', '바닥 확인 중', '다리 재부팅 중', '말랑하게 도착', '안 터진 척'];
const BOUNCE_LINES = ['통!', '퐁!', '또 튄다', '나 돌아왔어', '벽 있었네', '말랑 반사', '몸이 튕기는 중', '벽 확인 중', '통통 배송 중', '다시 오는 중'];
